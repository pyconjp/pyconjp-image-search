"""SigLIP model wrapper for image and text embedding."""
...
class SigLIPEmbedder:
    ...
    def __init__(
        ...
        self.model = SiglipModel.from_pretrained(model_name).to(device).eval()  # type: ignore[arg-type]
    ...
    def _extract_embeddings(outputs: object) -> np.ndarray:
        ...
        if hasattr(outputs, "pooler_output"):
            return outputs.pooler_output.cpu().numpy()  # type: ignore[union-attr]
        return outputs.cpu().numpy()  # type: ignore[union-attr]
